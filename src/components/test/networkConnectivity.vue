...
<script>
  import {getNetworkConnectivityData} from "../../request/api";
  ...
  export default {
    ...
    methods: {
      getList() {
        let that = this;
        getNetworkConnectivityData(this.testId1,this.controlCenterId).then((response) => {
            console.log("获取测试信息成功");
            console.log(response);
            this.tableData1 = response.data;
            var obj = this.tableData1;
            sessionStorage.setItem("networkConnectivityData", JSON.stringify(obj));
    ...
    created() {
      ...
      var read = JSON.parse(sessionStorage.getItem("networkConnectivityData"));
